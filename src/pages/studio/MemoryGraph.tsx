...
export default function MemoryGraph({ workspaceId, missionId, onSelectNode }: Props) {
  ...
  return <section className="card memory-graph stack" aria-labelledby="memory-graph-title">
    <div className="row" style={{ justifyContent: 'space-between' }}><div><h2 id="memory-graph-title" className="subhead">Memory graph</h2><p className="label" style={{ margin: 0 }}>Persisted relationships · {graph.nodes.length} nodes · {graph.edges.length} edges</p></div><span className="sticker">v{graph.createdAt ? new Date(graph.createdAt).toLocaleDateString() : 'live'}</span></div>
    {graph.nodes.length === 0 ? <p className="label">No persisted records yet. Add a memory or source to see it here.</p> : <>
      <div className="memory-graph-canvas" role="img" aria-label="Memory graph visualization. Use the table below to select a node.">
        <svg viewBox="0 0 800 260" aria-hidden="true" focusable="false">{graph.edges.map((edge) => { const a = graph.nodes.findIndex((n) => n.id === edge.source); const b = graph.nodes.findIndex((n) => n.id === edge.target); if (a < 0 || b < 0) return null; const x1 = 80 + (a % 6) * 130; const y1 = 55 + Math.floor(a / 6) * 85; const x2 = 80 + (b % 6) * 130; const y2 = 55 + Math.floor(b / 6) * 85; return <line key={edge.id} x1={x1} y1={y1} x2={x2} y2={y2} stroke="currentColor" strokeOpacity=".28" />; })}{graph.nodes.map((node, index) => { const x = 80 + (index % 6) * 130; const y = 55 + Math.floor(index / 6) * 85; return <g key={node.id} transform={`translate(${x} ${y})`}><circle r="24" fill="currentColor" opacity={selected === node.id ? 0.9 : 0.25} /><text textAnchor="middle" y="4" fontSize="9" fill="currentColor">{node.kind.slice(0, 8)}</text></g>; })}</svg>
      </div>
      <div className="table-scroll"><table className="data-table"><caption className="sr-only">Synchronized memory graph nodes</caption><thead><tr><th scope="col">Node</th><th scope="col">Kind</th><th scope="col">Status</th><th scope="col">Provenance</th></tr></thead><tbody>{graph.nodes.map((node) => { const provenance = node.provenance ?? []; return <tr key={node.id} className={selected === node.id ? 'is-selected' : ''}><td><button type="button" className="btn btn-sm" onClick={() => select(node.id)}>{labelFor(node)}</button></td><td>{node.kind}</td><td>{node.status ?? 'recorded'}{node.revision ? ` · r${node.revision}` : ''}</td><td>{provenance.length ? `${provenance.length} source${provenance.length === 1 ? '' : 's'}` : '—'}</td></tr>; })}</tbody></table></div>
    </>}

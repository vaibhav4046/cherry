...
import { approvalRequired, conflict, invalid, notFound, unsupported } from '../core/errors.ts';
...
export async function requestSourceFetch(sourceId: string, actorType: ActorType = 'human'): Promise<Result<SourceRecord>> {
  ...
  return withWorkspaceTx(current.workspaceId, ['sourceRecords'], async (ctx) => {
    ...
    if (anchor.kind !== 'article') return unsupported('Only saved article sources can use the local page fetcher', { sourceId, kind: anchor.kind });
    if (!anchor.permissionAcknowledgedAt) return approvalRequired('A person must acknowledge permission before fetching this source', { sourceId });
    if (anchor.fetchStatus === 'queued') return conflict('A fetch is already queued for this source', { sourceId });
    if (anchor.fetchStatus === 'fetched') return conflict('This source already has fetched content', { sourceId });

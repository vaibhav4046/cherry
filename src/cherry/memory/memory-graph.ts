...
export async function buildMemoryGraph(workspaceId: string, missionId?: string | null): Promise<Result<MemoryGraph>> {
  ...
  for (const g of graphs.filter((x) => graphIds.has(x.id))) { for (const n of g.nodes) { add(n.id, 'skill-node', 'skillNode', { ...n, missionId: g.missionId, revision: g.revision }); for (const e of n.evidenceIds ?? []) edge(`${e}->${n.id}`, 'evidence→skill-node', e, n.id); } for (const v of skillVersions.filter((x) => x.skillGraphId === g.id && x.status === 'approved')) add(v.id, 'approved-skill-revision', 'skillGraphVersion', { ...v, missionId: g.missionId, status: v.status, revision: v.revision, approvedRevision: v.snapshot.approvedRevision ?? v.revision }, g.name); }
  ...
  const artifacts = await db.artifactSets.where('workspaceId').equals(workspaceId).toArray(); for (const a of artifacts.filter((x) => !missionId || x.missionId === missionId)) { const files = await db.artifactFiles.where('artifactSetId').equals(a.id).toArray(); for (const f of files) { add(f.id, 'artifact', 'artifactFile', f, f.path); } }

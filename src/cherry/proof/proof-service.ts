...
export async function createProofReceipt(missionId: string): Promise<Result<ProofReceipt>> {
  ...
  const missionEvents = events.filter((event) => {
    ...
    if (causalIds.has(event.objectId) || (event.objectType === 'run' && missionRunIds.has(event.objectId))) return true;
    const createdSourceIds = event.payload?.createdSourceIds;
    return Array.isArray(createdSourceIds)
      && createdSourceIds.some((sourceId) => typeof sourceId === 'string' && linkedSourceIds.has(sourceId));
  });

...
const DEFAULT_REPLAY_TRUST = resolve('src/components/showcase/recorded-mission-trust.mjs');
...
export async function writePublicReplay(
  sourcePath = DEFAULT_SOURCE,
  outputPath = DEFAULT_REPLAY,
  trustPath = DEFAULT_REPLAY_TRUST,
) {
  const captureText = await readFile(sourcePath, 'utf8');
  const replay = await buildRecordedMissionFixture(captureText);
  if (!(await verifyRecordedMissionFixture(replay, replay.integrity.replaySha256))) {
    throw new Error('Refusing to write an unverified recorded mission replay.');
  }
  const bytes = `${JSON.stringify(replay, null, 2)}\n`;
  const trustBytes = '// Generated by scripts/capture-winner-demo.mjs; do not hand-edit.\n'
    + `export const RECORDED_MISSION_EXPECTED_SHA256 = '${replay.integrity.replaySha256}';\n`;
  await replaceAtomically(outputPath, bytes);
  await replaceAtomically(trustPath, trustBytes);
  return {
    outputPath,
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (argument === '--trust-output') options.trustPath = resolve(argv[++index]);
...
async function main() {
  ...
  const replay = await writePublicReplay(options.sourcePath, options.replayPath, options.trustPath);
